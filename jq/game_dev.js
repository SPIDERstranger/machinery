...
/**
 * 电源上限自动升级  
 * not set in Init(), changed through prestige  
 * 0-手动  
 * 1-自动 
 */
...
/**
 * 暂未使用
 * generator actions per cycle (currently unused)
 */
var actions_cycle = 0;//generator actions per cycle (currently unused)
/**
 * 研究实验室提供的升级能量倍率
 */
var bonus_multiplier;//part of moneyCalc() formula// 研究实验室倍率
...
/**
 * 楼层管理员rank2的第一个项目是否购买
 */
var warp_rank2_training1_flag = 0;
/**
 * 楼层管理员rank2的第二个项目是否购买
 */
...
/**
 * 当前第一台发电机供应升级百分比0~100
 */
var one_upgrade_supply_limit_stage;
/**
 * 当前第一台发电机电源升级百分比0~100
 */
...
/**
 * 当前第二台发电机供应升级百分比0~100
 */
var two_upgrade_supply_limit_stage;
/**
 * 当前第二台发电机电源升级百分比0~100
 */
...
/**
 * 当前第三台发电机供应升级百分比0~100
 */
var three_upgrade_supply_limit_stage;
/**
 * 当前第三台发电机电源升级百分比0~100
 */
...
/**
 * 当前第四台发电机供应升级百分比0~100
 */
var four_upgrade_supply_limit_stage;
/**
 * 当前第四台发电机电源升级百分比0~100
 */
...
var foundry_state = 0;
/**
 * 工程巢穴 工程师等级,是否开启,自动购买电源上限,以及开启辅助电源
 */
...
/**
 * 确认扭曲后,对当前轮回进行收尾工作,并进行初始化
 */
function prestigeOk() {
  //LOOPS
  clearInterval(telescope_timer); telescope_timer = null;//stop the telescope 暂停电报的计时器
  clearInterval(magnetron_interval); magnetron_interval = null;//stop the magnetron 暂停磁控管计时器
  clearInterval(save_timer); button3Disable(save_upgrade); save_timer = null;//stop the save timer,  暂停自动保存计时器
  clearInterval(furnace_cooling_timer); furnace_cooling_timer = null;// 暂停铸造厂的冷却计时器
  ...
  if (ultimate_ratio < 100) { recency = 1; }//过快进行重启扭曲
  else { recency = 0; }//recency=0 is what you want//达到100%进行扭曲
...
function prestigeState() {
  ...
  if (engden_state == 1) {//工程师等级的购买按钮
    warp_rank1_upgrade.text("Sold");
    button8Disable(warp_rank1_upgrade);
    warp_rank2_training1_upgrade.show();
  }
  if (warp_rank2_training1_flag == 1 || quantum_upgrade_flag[1] == 1) {//楼层管理员等级第一个购买项目
  ...
  if (warp_rank2_training2_flag == 1 || quantum_upgrade_flag[1] == 1) {//楼层管理员等级第二个购买项目
    warp_rank2_training2_upgrade.text("Sold");
    button8Disable(warp_rank2_training2_upgrade);
    warp_rank2_upgrade.show();
  }
  if (lscanner_state == 1 || quantum_upgrade_flag[1] == 1) {//楼层管理员等级第三个购买项目
...
/**
 * 根据电源厂是否自动运营,更新自动化相关开关
 */
function ccSetup() {
  if (chief_check == 1) {//如果电源厂自动运营开启
...
function researchRequest() {//this generates a new item (no UI elements)
  ...
  if (researchRNG.random[research_playhead] < research_probability) {//most are power increases
    ...
    if (lscanner_state == 1 && recency == 0) {//when you unlock lscanner and if recency==0, you get a lifeform
  ...
  for (let i = 0; i < 8; i++) {
    ...
    switch (i) {
      ...
      case 2:
        ...
        //display
        if (animal3_magnetron_multiplier > 0) {
          animal3_magnetron_multiplier_label.text('+' + animal3_magnetron_multiplier);
 ...
 * @param {Number} next_gen_price 下次购买价格
 * @param {Number} power_stage 
 * @returns 
 */
...
//this is done due to the behavior of progress bars, 
//which don't seem to be visually updated if the div of the tab is hidden;
//therefore, I call these functions in color_block. 
//The problem would be especially noticeable when auto-buy is on
//the downside of this solution is that it duplicates code from 
//the x_upgrade_effectiveness functions 
//and in theory I could've simply moved all the UI functions here, 
//so that I call these functions as well. 
//The pedantic upside is that I've removed a couple of calls in these ones
 ...
 * @param {} $element 进度条元素
 * @param {} $element_indicator 进度条进度元素
 * @param {String} label 进度条上方标签
 */
